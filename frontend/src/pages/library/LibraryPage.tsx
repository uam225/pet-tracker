...
import { Card, Badge, Button, Input, Textarea, Modal, EmptyState, Fab, SectionHeader } from '@/components/ui'
import { BookOpen, Plus, Search, AlertTriangle, ChevronRight, Archive, X } from 'lucide-react'
import type {
...
const INGREDIENT_CATEGORIES: IngredientCategory[] = [
  'Poultry', 'Fish', 'Grain', 'Dairy', 'Vegetable', 'Fruit', 'Supplement', 'Other',
]
    ...
    >
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          <span className="text-xs text-stone-300">{item.ingredients.length} ingredient{item.ingredients.length !== 1 ? 's' : ''}</span>
        </div>
      </div>
...
function IngredientPicker({ allIngredients, selected, onChange }: {
    ...
    <div className="flex flex-col gap-2">
      ...
      {search && (
        <div className="max-h-40 overflow-y-auto flex flex-col border border-stone-200 rounded-xl divide-y divide-stone-100">
          ...
          {filtered.length === 0 && <p className="text-xs text-stone-400 px-3 py-2">No results. Create the ingredient first.</p>}
        </div>
      )}
    </div>
  )
...
function FoodItemModal({ item, categories, allIngredients, onClose }: {
  ...
  const [ingredients, setIngredients] = useState<FoodItemIngredientCreate[]>(
    item?.ingredients.map(i => ({ ingredient_id: i.ingredient_id, percentage: i.percentage ?? undefined })) ?? []
  )
  const [error, setError] = useState('')

  const createMutation = useMutation({
    mutationFn: () => foodApi.createItem({ name: name.trim(), brand: brand || undefined, food_category_id: catId, description: desc || undefined, ingredients }),
  ...
  const updateMutation = useMutation({
    mutationFn: () => foodApi.updateItem(item!.id, { name: name.trim(), brand: brand || undefined, food_category_id: catId, description: desc || undefined, ingredients }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['food-items'] }); onClose() },
    onError: (e: unknown) => setError(e instanceof ApiRequestError ? e.detail : 'Failed to save.'),
  })
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        <Input label="Name" value={name} onChange={e => setName(e.target.value)} required placeholder="e.g. Acana Grasslands Kibble" />
        <Input label="Brand (optional)" value={brand} onChange={e => setBrand(e.target.value)} placeholder="e.g. Acana" />
            ...
            {categories.map(cat => (
              <button
                ...
                className={['px-3 py-1.5 rounded-xl text-sm font-medium transition-colors',
                  catId === cat.id ? 'bg-forest-500 text-white' : 'bg-stone-100 text-stone-600'
                ].join(' ')}
              >
                {cat.name}
              </button>
        ...
        <Textarea label="Description (optional)" value={desc} onChange={e => setDesc(e.target.value)} placeholder="Any notes about this product…" />

        <div className="flex flex-col gap-2">
          <label className="text-sm font-medium text-forest-800">Ingredients</label>
          <IngredientPicker allIngredients={allIngredients} selected={ingredients} onChange={setIngredients} />
        </div>
